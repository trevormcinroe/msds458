...
## Introduction
The XOR problem is a problem that has been tackled in classic deep learning theory. "XOR" is short of "exclusive or". 
In this context, it is an operation between two binary inputs that produces a "1" when the two inputs are different and
a "0" if the two outputs are the same. This result s in the following data:
...
This is an important problem because the two classes for `y` are not linearly separable. 

Based on some earlier research by McCulloch and Pitts (1943), Frank Rosenblatt developed the first idea of a 
"perceptron" in his book _Principles of Neurodynamics_ (1962). The original perceptron had a single layer where the 
inputs, ![](https://latex.codecogs.com/gif.latex?%5Cdpi%7B120%7D%20x_i), were multiplied by their own weight,
...
problems. Simply by adding an addition "hidden" between the input and output layers that passes its product 
...
single hidden layer containing a finite number of neurons can approximate any arbitrary real-values continuous function
on compact sets of ![](https://latex.codecogs.com/gif.latex?%5Cdpi%7B120%7D%20%5Cmathbb%7BR%7D%5En)". This theorem was
proven to be true for networks with sigmoid activation functions in 1989 by George Cybenko (http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.441.7873&rep=rep1&type=pdf)
